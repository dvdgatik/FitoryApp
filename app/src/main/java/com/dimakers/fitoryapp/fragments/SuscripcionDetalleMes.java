...
public class SuscripcionDetalleMes extends Fragment {
    ...
    public static SuscripcionDetalleMes newInstance() {
        SuscripcionDetalleMes misSuscripciones = new SuscripcionDetalleMes();
    ...
    public void onViewCreated(@NonNull View view, @Nullable Bundle savedInstanceState) {
        ...
        btnCancelarSuscripcion = view.findViewById(R.id.btn_renovar_suscripcion);
        ...
        if (bundle != null) {
            SubscripcionMes subscripcionMes = (SubscripcionMes) bundle.getSerializable(Variables.SUSCRIPCION);
            Call<Visita> visitaCall = service.obtenerVisitas(subscripcionMes.getSucursal(),subscripcionMes.getCliente());
...
//
//            }
//            tvSuscripcionSesiones.setText(suscripcion.getSesion().getSesiones()+" Sesiones");
//            tvSuscripcionPrecio.setText("$"+suscripcion.getSesion().getTotal()+" MXN");
//            tvSuscripcionSesionesRestantes.setText("Sesiones Restantes: "+suscripcion.getSesion().getSesionesRestantes());
            tvSuscripcionPrecio.setText("$"+subscripcionMes.getTotalCobrar()+" MXN");
            tvSuscripcionSesionesRestantes.setText(subscripcionMes.getFechaRenovacion());
            tvSuscripcionComoLlegar.setOnClickListener(new View.OnClickListener() {
                @Override
                public void onClick(View v) {
                    String latitud = subscripcionMes.getSucursalLatitud();
                    String longitud = subscripcionMes.getSucursalLongitud();
            ...
            btnCancelarSuscripcion.setOnClickListener(new View.OnClickListener() {
                ...
                public void onClick(View v) {
                    ...
                    if (clienteID==0) {
                        Toast.makeText(getContext(), "No se pudo encontrar un cliente id", Toast.LENGTH_SHORT).show();
                    } else {
                        Call<ResponseBody> cancelarSuscripcionCall = service.cancelarSubscripcion(clienteID,subscripcionMes.getId());
                        cancelarSuscripcionCall.enqueue(new Callback<ResponseBody>() {
                            @Override
                            public void onResponse(Call<ResponseBody> call, Response<ResponseBody> response) {
                                if (response.isSuccessful()) {
                                    ...
                                    Toast.makeText(getContext(), "Error al cancelar la suscripción", Toast.LENGTH_SHORT).show();
                                }
                            }
            ...
            //Detalles del club
            tvSuscripcionDireccion.setText(subscripcionMes.getSucursalDireccion());
            tvSuscripcionTelefono.setText(subscripcionMes.getSucursalTelefono());
            tvSuscripcionCorreo.setText(subscripcionMes.getSucursalCorreo());

            Call<PerfilSucursal> perfilSucursalCall = service.obtenerPerfilSucursal(subscripcionMes.getSucursal(),subscripcionMes.getCliente());
...
//        adapter.update(suscripcions);
    ...
    public View onCreateView(@NonNull LayoutInflater inflater, @Nullable ViewGroup container, @Nullable Bundle savedInstanceState) {
        return inflater.inflate(R.layout.fragment_suscripcion_detalle_mes,container,false);
    }
